...
from surfex.file import (
    AsciiSurfexFile, NCSurfexFile, ForcingFileNetCDF,
    SurfexFileVariable, TexteSurfexFile, FaSurfexFile,
    NetCDFSurfexFile,
    read_surfex_field,
    read_surfex_points
)
...
def test_read_ascii_record_conf_proj_geo_provided(ascii_conf_proj_float_record_file, conf_proj_2x3):
...
def test_read_surfex_field(ascii_conf_proj_float_record_file):
    ...
    field = read_surfex_field(
        "TG1P1", ascii_conf_proj_float_record_file, validtime=validtime,
        fileformat="ascii", filetype="surf", datatype="float",
        tiletype="NATURE"
    )
...
def test_read_surfex_points(ascii_conf_proj_float_record_file, conf_proj_2x3):
    ...
    field = read_surfex_points(
        "TG1P1", ascii_conf_proj_float_record_file, conf_proj_2x3,
        validtime=validtime, fileformat="ascii", filetype="surf",
        tiletype="NATURE", datatype="float"
    )
    assert field.shape[0] == 6
...
def test_read_fa_field(fa_conf_proj_file, conf_proj_2x3):
    ...
    __ = fa_file.points(var, conf_proj_2x3)
...
def test_timeseries_netcdf_file(conf_proj_2x3, data_timeseries_netcdf_file):
    ...
    with pytest.raises(IndexError):
        __ = nc_file.points(var, conf_proj_2x3, validtime=validtime)
