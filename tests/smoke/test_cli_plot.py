...
from pysurfex.cli import plot_points, cli_set_geo_from_obs_set
...
def test_plot_grib1(tmp_path_factory, conf_proj_2x3_file, lambert_t2m_grib1):
    ...
    argv = [
        "-g", conf_proj_2x3_file,
        "-o", output_file,
        "--debug",
        "--inputtype", "grib1",
        "--basetime", "2020022006",
        "--validtime", "2020022006",
        "--indicatorOfParameter", "11",
        "--level", "2",
        "--levelType", "105",
        "--inputfile", lambert_t2m_grib1,
    ]
...
def test_plot_netcdf(tmp_path_factory, conf_proj_2x3_file, data_thredds_nc_file):
    ...
    argv = [
        ...
        "--inputfile", data_thredds_nc_file,
    ]
    plot_points(argv=argv)
...
def fixture_geo_from_obsset(tmp_path_factory, obsset_fname, obstime_str):
    output_file = (
        f"{tmp_path_factory.getbasetemp().as_posix()}/geo_from_obsset.json"
    )
    argv = [
        "-v", "air_temperature",
        "-i", obsset_fname,
...
@pytest.mark.usefixtures("_mockers")
def test_plot_obs_frost_json(tmp_path_factory, obsset_fname, obstime_str, geo_from_obsset):
    ...
    argv = [
        ...
        "--inputtype", "obs",
        "--obs-type", "json",
        "--basetime", obstime_str,
        "--validtime", obstime_str,
        "--variable", "air_temperature",
        "--inputfile", obsset_fname,
    ]
    plot_points(argv=argv)
