...
class PGDClient(Client):
    ...
    def parse(self):
        ...
        parser.add_argument('--json', '-j', type=str, nargs="+", default=None, help="A JSON file with run options")
        parser.add_argument('--toml', '-t', type=str, default=None, help="A TOML configuration file to be parsed to json file with run options")
    ...
    def run(self):
        ...
        elif self.toml is not None:
            json_settings = surfex.TomlFile2Json(self.toml).json_settings
            print(type(json_settings))
            print(json_settings)
            print("2:", json.loads(json_settings))
        else:
            print("You must specify either a json file or a toml file!")
            raise Exception
