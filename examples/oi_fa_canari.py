...
if __name__ == "__main__":
    ...
    if format == "fa" and my_settings["nam_io_offline"]["lfagmap"]:
        ext = ".sfx"
    ...
    pgd = test_dir + "/archive/oi_"+format+"/"+my_settings["nam_io_offline"]["cpgdfile"]+ext
    ...
    # PREP
    prep = test_dir + "/archive/oi_"+format+"/"+my_settings["nam_io_offline"]["cprepfile"]+ext
    ...
    first_guess = test_dir+"/archive/oi_"+format+"/"+my_settings["nam_io_offline"]["cpgdfile"]+ext
    ...
    my_input = [my_ecoclimap_input]
    canari = test_dir+"/archive/oi_"+format+"/ICMSHHARM+0000"+ext
    if not os.path.exists(canari):
        workdir = test_dir + "/canari"
        surfex.create_working_dir(workdir)
        my_pgdfile = surfex.PGDFile(format, my_settings["nam_io_offline"]["cpgdfile"], my_geo, input_file=pgd,)
        my_prepfile = surfex.PREPFile(format, my_settings["nam_io_offline"]["cprepfile"], my_geo, input_file=prep)
        my_canarifile = surfex.SURFFile(format, my_settings["nam_io_offline"]["csurffile"], my_geo, archive_file=canari)

        #binary = test_dir + "/bin/MASTERODB"
        binary = None
        surfex.Canari(my_settings, my_batch, my_pgdfile, my_prepfile, my_canarifile, assim, binary=binary,
                      input=my_input, print_namelist=True)
        # surfex.clean_working_dir(workdir)
